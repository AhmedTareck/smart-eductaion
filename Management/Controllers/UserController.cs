...
namespace Management.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpGet("getUser")]
        public IActionResult getUser(int pageNo, int pageSize,int UserType,long id)//type of id 
        {
            try
            {
                ...
                if(id!=0)
                {
                    Users = from p in Users where p.UserId == id select p;
                }
        ...
        [HttpGet("getUserName")]
        public IActionResult getUserName(int UserType) 
        {
            try
            {
                IQueryable<Users> Users = from p in db.Users where p.State != 9 && p.UserType==UserType select p;
